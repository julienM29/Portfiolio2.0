...
function Contact({ isLight }) {
  ...
  const submitMailForm = useCallback(async (dataFormMail) => {
    try {
      ...
      toast.error(
        <p className="titre text-light-primary">Erreur réseau, veuillez réessayer !</p>,
  ...
  return (
    ...
    >
      ...
      <motion.h2 variants={childVariants} className={`titre font-bold text-3xl md:text-4xl text-center sm:text-left ${isLight ? 'text-light-primary' : 'text-dark-primary'}`}>
        Contact
      </motion.h2>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <motion.div variants={childVariants}>
              <label htmlFor="prenom" className="block text-base font-medium mb-2">Prénom*</label>
              <Input
                id="prenom"
                name="prenom"
                type="text"
                placeholder="Prénom"
                required
                className={`rounded-xl p-4 text-base font-medium border shadow-sm focus:outline-none focus:ring-2 ${isLight ? 'bg-white text-gray-900 border-light focus:ring-light-accent' : 'bg-dark-text-primary text-black border-dark-composant focus:ring-dark-accent'}`}
              />
            </motion.div>

            <motion.div variants={childVariants}>
              <label htmlFor="nom" className="block text-base font-medium mb-2">Nom*</label>
              <Input
                id="nom"
                name="nom"
                type="text"
          ...
          <motion.div variants={childVariants}>
            ...
            <Input
              ...
              className={`rounded-xl p-4 text-base font-medium border shadow-sm w-full focus:outline-none focus:ring-2 ${isLight ? 'bg-white text-gray-900 border-light focus:ring-light-accent' : 'bg-dark-text-primary text-black border-dark-composant focus:ring-dark-accent'}`}
            />
          ...
          <motion.div variants={childVariants} className="w-full flex justify-center">
            <button
              ...
              className={`hover:scale-[1.02] font-semibold px-4 py-1.5 md:px-6 md:py-3 rounded-xl border-2 cursor-pointer transition-transform duration-100 active:shadow-none active:translate-x-[3px] active:translate-y-[3px] ${isLight ? 'bg-light-bouton border-black text-black' : 'bg-dark-bouton border-dark-bouton text-dark-primary'}`}
              style={{ boxShadow: `2px 2px 0 0 ${isLight ? '#000000' : '#EEEEEE'}` }}
            >
              Envoyer le message
            </button>
          </motion.div>
